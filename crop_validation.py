...
def resize_face(colorfull, gray, hog_face_detector, face_landmarks):
    ...
    for i in landmark_coords:
        cv2.circle(colorfull, i, 1, (0, 0, 255))
    cv2.imshow("After align and before resize", colorfull)
    cv2.waitKey(0)
    ...
    for i in landmark_coords:
        try:
            cv2.circle(resized_colorfull, i, 1, (0, 0, 255))
        except Exception as e:
            print(f"An error occurred: {e}")
    cv2.imshow("resized and cropped", resized_colorfull)
    cv2.waitKey(0)
...
"""
    for ind in range(11):
        print(ind, end=":")
        print("average:", sum(allvectors[ind]) / len(allvectors[ind]), ", min:", min(allvectors[ind]), ", max:", max(allvectors[ind]))
"""