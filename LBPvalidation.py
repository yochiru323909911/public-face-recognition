...
NUM_FEATURES = 256 + 10  # The histogram and the regular features
IRIS_TOLARENCE = 20
IMAGE_WIDTH = 500
LEFT = 0
RIGHT = 1
...
def detect_features(gray, hog_face_detector, landmark_coords):
...
def compare_features(f1, f2, vec):
    vec.append(np.sqrt(((f1[0]-f2[0])**2+(f1[1]-f2[1])**2)))
...
def get_nose_mean_color(image, nose):
    ...
    # Print the results
    print("Mean Gray Color:", mean_gray)
    print("A:", mean_a)
    print("B:", mean_b)
    print("Mean H:", mean_h)
    print("Mean S:", mean_s)
...
def recognize_face(image_name, hog_face_detector, face_landmarks):
    ...
    final_color = cv2.imread(image_name)
    final_gray = cv2.cvtColor(final_color, cv2.COLOR_BGR2GRAY)
    final_lab = cv2.cvtColor(final_color, cv2.COLOR_BGR2LAB)
    l, a, b = cv2.split(final_lab)
    l = l * 0
    # Merge the modified LAB channels
    modified_lab_image = cv2.merge((l, a, b))
    # Convert the LAB image back to the original color space (e.g., RGB)
    modified_image = cv2.cvtColor(modified_lab_image, cv2.COLOR_LAB2BGR)
    # Display the modified image
    cv2.imshow('Orginal Image', final_color)
    cv2.waitKey(0)
    cv2.imshow('Modified Image', modified_image)
    cv2.waitKey(0)
    cv2.imshow('Modified LAB Image', modified_lab_image)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

    print("All mean A:", np.mean(a))
    print("All mean B:", np.mean(b))
    final_gray, final_color, landmarks_coords = resize_face(final_color, final_gray, hog_face_detector, face_landmarks)

    # Detect facial features
    eyes, nose, mouth, eyebrow, jaw = detect_features(final_gray, hog_face_detector, landmarks_coords)
    features = get_nose_mean_color(final_color, nose)

    features.extend(extract_features(eyes, nose, mouth, eyebrow, jaw))
...
def show_visually(vector1, vector2):

    v1 = np.array(vector1)
    v2 = np.array(vector2)
    plt.scatter(v1, np.zeros_like(v1), c='r', label='user 1')
    plt.scatter(v2, np.zeros_like(v2), c='b', label='user 2')
    ...
    for sublist in allvectors:
        sublist.clear()
# ======================================================================================================================
def recognize(user, hog_face_detector, dlib_facelandmark):
    f1 = []
    image_paths = []
    with open('faces/{}.txt'.format(user), 'r') as f:
        image_paths = ["faces/{}/".format(user) + line.strip() + ".jpg" for line in f]
...
def compare_color_noses(f1, f2):
    first, second, vs = [], [], []
    print("first:")
    for i in range(len(f1) - 1):
        print(compare_features(f1[i], f1[i + 1], first))
    print("second:")
    for i in range(len(f2) - 1):
        print(compare_features(f2[i], f2[i + 1], second))
    print("first vs second:")
    for i in range(min(len(f1), len(f2))):
        print(compare_features(f1[i], f2[i], vs))

    show_visually(first, vs)
    show_visually(second, vs)
# ======================================================================================================================
...
if __name__ == "__main__":
    ...
    print("40")
    recognize_face("faces/y/y40.jpg", hog_face_detector, dlib_facelandmark)
    print("41")
    recognize_face("faces/y/y41.jpg", hog_face_detector, dlib_facelandmark)
    print("42")
    recognize_face("faces/y/y42.jpg", hog_face_detector, dlib_facelandmark)
    print("43")
    recognize_face("faces/y/y43.jpg", hog_face_detector, dlib_facelandmark)


16.155577230050657772258496869475
8.6333879592151455008835201615228
82.8323049790695001985368498125
9.1012254657858850452574319621111

11.038928989607237581682863432155
19.137453012797197285693382055947

8.1124906928602223636384707293046
